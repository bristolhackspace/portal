...
class UsernameForm(FlaskForm):
    username = StringField(
        ...
        validators=[
            ...
            Regexp(
                r"^[\w-]+$",
                message="Username can only contain a-z, A-Z, 0-9 '_' and '-' characters.",
            ),
...
class OtpForm(FlaskForm):
    otp = StringField(
        "Please enter the code sent to your email:", validators=[DataRequired()]
    )
...
def index():
    ...
    if (
        flow is None
        or flow.next_step() == FlowStep.NOT_STARTED
        or "resend_email" in request.args
    ):
        data = {}
    ...
    if step == FlowStep.VERIFY_EMAIL:
        ...
        return render_template(
            "login/otp.html.j2", flow=flow, form=form, resend_email_url=resend_email_url
        )
